...
@section('content')
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    @php
                     ...
                     $advaceAmount = 0;
                    @endphp
                    @if(count($reciveamount)>0)
                        @foreach ($reciveamount as $reciveamount)
                            @php
                                $reciveamountCount = $reciveamountCount + $reciveamount->amount;
                            @endphp
                        @endforeach
                    @endif
                     @if(count($advance)>0)
                        @foreach ($advance as $advance)
                            @php
                                $advaceAmount = $advaceAmount + $advance->total;
                            @endphp
                        @endforeach
                    @endif
                     @php
                    ...
                    @endforeach
                    @php
                        $total = $paidamount + $unpaidamount + $advaceAmount;
                    @endphp
                    <div class="total_amount">
                        <div class="amount_div">
                            <p style="color: rgb(97, 97, 97); font-weight: 600; font-size: 13px;line-height: 23px;">Sale
                            </p>
                        ...
                        <div class="amount_div" style="background-color: #aed6f3;">
                            ...
                            <p style="color:rgb(5, 5, 43);font-weight: 600; font-size: 21px;line-height: 23px;">
                                {{ env('CURRENCY') }}{{ number_format($reciveamountCount, 2) }}
                            </p>
                        </div>
                        <!--<div class="icon_div">-->
                        ...
                        <div class="amount_div" style="background-color: #fbe88c;">
                            <p style="color: rgb(97, 97, 97); font-weight: 600; font-size: 13px;line-height: 23px;">
                                Due</p>
                            <p style="color:rgb(5, 5, 43);font-weight: 600; font-size: 21px;line-height: 23px;">
                                {{ env('CURRENCY') }}{{ number_format($total-$reciveamountCount, 2) }}
                            </p>
                        </div>
                        <!--<div class="amount_div" style="background-color: #fbe88c;">-->
                        <!--    <p style="color: rgb(97, 97, 97); font-weight: 600; font-size: 13px;line-height: 23px;">-->
                    ...
                    <table id="selltable" width="100%" class="datatable-init table"
                        ...
                        <thead>
                            <tr>
                                ...
                                <th>
                                    <span class="overline-title">Sale ID</span>
                                </th>
                                <th>
    ...
    <script>
        document.getElementById("sell-pdf").addEventListener("click", function() {
            ...
            html2canvas(table).then(function(canvas) {
                ...
                pdf.save("Sell-report.pdf");
            });
        });

        document.getElementById("sell-excel").addEventListener("click", function() {
            var table = document.getElementById("selltable");
            var wb = XLSX.utils.table_to_book(table, {
                sheet: "Sheet1"
            });
            XLSX.writeFile(wb, "Sell-report.xlsx");
        });
    </script>
    <script>
